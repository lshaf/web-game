...
const phase = ref('setup') // setup | handoff | play | won | lost
const clue = ref('')
const answer = ref('')
...
function submit() {
  ...
  typed.value = ''
  if (guess === answer.value) phase.value = 'won'
  else if (guesses.value.length >= maxAttempts.value) phase.value = 'lost'
